...
| 数据层 | 电脑 | 10 源 + SQLite 缓存 | `store.py` + `sources/` | ✅ |
| 全局状态层 | 口袋 | 持仓/余额/订单 | — | ❌ |
...
**已完成：** 数据层（10 源 + 缓存）、client.py 拆为纯路由+缓存包装、发现层+时机层雏形、全局调度层。100 轮压力测试：预热后 100% 缓存命中率，冷启动 84%。2026-06-20 从 a-stock-data V3.2.4 吸收 4 个新源（东财研报/一致预期/龙虎榜/公告），总源数 6 → 10。
